...
import erppeek
import postgreLib
import odoorpc #Load base language library
import logging
from utills import *
...
class odooLibrary:
    ...
    def __init__(self, dbIp, dbPort, dbusername = None, dbpwd = None, dbName = None, portal_port = None, admin_name = None):
        ...
        self.status = "starting the process"
    ...
    def get_currentStatus(self):
        try:
            ...
            logging.warning("[WARNING] GET CURRENT STATUS EXCEPTION ERROR :: %s"%er)
    ...
    def load_base_language(self, databasename):
        try:
            langs = ['es_ES', 'pt_BR', 'en_US']
            odoo = odoorpc.ODOO(self.dbIp, 'jsonrpc' , self.portal_port)
            odoo.login(databasename, 'admin', self.dbpwd)
            base_lng_model = odoo.env['base.language.install']

            for lang in langs:
                id = base_lng_model.create({'lang':lang,'state': 'done', 'overwrite':True})
                result = base_lng_model.lang_install(id)
             
        except Exception as er:
            logging.warning("[WARNING] INSTALLATION BASE LANGUAGE EXCEPTION :: %s"%er)
    ...
    def get_newDatabase_admin_id(self, database, params=None):
        try:
            ...
            self.status = "Database user id creation in-progress"

            if user_id is None:
                logging.warning("[WARNING] ADMIN USER NAME NOT FOUND IN CURRENT DATABASE " + database +" TRY AFTER SOMETIME")
            else:
                update_pwd = self.connection.reset_admin_pwd(self.connect, user_id, self.defaultpwd)
        ...
        try:
            ...
            for insta_module in module_name:
                ...
                if insta_module in modules['uninstalled']:
                    self.status = "%s Module installation in-progress"%insta_module.upper()
                    client.install(insta_module)
                    logging.info("[SUCCESS] THE MODULE %s HAS BEEN INSTALLED!"%insta_module)
                    time.sleep(5)
                    self.status = "%s Module installed successful"%insta_module.upper()
    ...
    def create_db_sub_users(self, database, params=None):
        try:
            ...
            if params is None:
                ...
                time.sleep(5)
                self.status = "Your user id created"

                if user_id is None:
                    logging.warning("[WARNING] USERNAME or EMAIL ID IS ALREADY EXIST IN ODOO SERVER")
                else:
                    self.connect_user_database(database)
                    random_pwd = params.get('client_password') 
                    print "random password %s"%random_pwd
                    update_pwd = self.connection.reset_admin_pwd(self.connect, user_id, random_pwd)
                    if update_pwd is not None:
                        self.create_usrInfoTable(params)
                        email_notification(client_name, body = "Dear Customer <br><br> we are proudly welcome to You <br><br> Your Account has been created succesfully your login Credentials <b><br> user name : %s <br> Password : %s <br></b>Note : if you create addon-subusers username is email id and password is user choose based  <br><br> Thanks & Regards<br>B2K Support Team"%(client_name, random_pwd))
                        self.status = "Email notification send successful"
                        time.sleep(5)
                        self.status = "Service request completed"
                    else:
    ...
    def sendFeedbackMail(self, params):
        try:  
            email_notification("renato.almeida@storaze.com.br", body="Hello  Postmaster </br></br> You have Received one New Feedback Message <br></br> Client Name :: %s <br> Client Company : %s <br> Customer mail-ID : %s <br> Customer Phone Number : %s <br> Message : %s  <br> </br> Thanks & Regards <br> B2K Support Team"%(params.get("custName"), params.get("custCompany"), params.get("custEmailAdd"), params.get("custPhone"), params.get("custmsg")))
        except Exception as er:
