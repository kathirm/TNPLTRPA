    ...
    for(var i=0;i<event.target.selectedOptions.length;i++){
        ...
        akey= Object.keys(productAmt)
            ...
            for(var j=0;j<akey.length;j++){
                if(event.target.selectedOptions[i].value == akey[j]){
                    pAmount = productAmt[akey[j]];
                }
            }
        var data =' <div class="col-sm-6"> '+event.target.selectedOptions[i].label+'</div><div class="col-sm-1 text-right">1</div> <div class="col-sm-5 text-right"><input type="text" style="width: 72%;text-align: center;" readonly="readonly" value="'+pAmount+'" id="box1"></div>'
            op1.innerHTML=op1.innerHTML + data;
        totAmt = Number(totAmt) + Number(pAmount);
        ary.push(event.target.selectedOptions[i].value);
    }
    obj["productType"]=ary;
