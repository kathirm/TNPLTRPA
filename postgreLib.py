...
class postgreLib:
    ...
    def reset_admin_pwd(self, conn, user_id, pwd):
        try:
            newpass_crypt = None;
            newpass_crypt = CryptContext(['pbkdf2_sha512']).encrypt(pwd)
    ...
    def create_table(self, conn):
        try:
            ...
            if self.cursor is not None:
                self.cursor.execute("CREATE TABLE %s(id serial PRIMARY KEY, client_name varchar, databaseName varchar, client_password varchar, productType varchar, totalAmount integer, CardNumber varchar, cardHolderName varchar, expiredMonth varchar, expiredYear integer, cardCVV integer, client_username varchar, user_limit integer, created_user integer, created_date varchar)"%self.res_usrinfo)
                conn.commit()
    ...
    def insertUsrRecords(self, conn, params=None):
        try:
            if params is not None:
                ...
                cur = conn.cursor()
                query = "INSERT INTO " + self.res_usrinfo +" (client_name, databaseName, client_password, productType, totalAmount, CardNumber, cardHolderName, expiredMonth, expiredYear, cardCVV, client_username,user_limit, created_user, created_date) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s','%s','%s')"%(params.get('client_name'), params.get('databaseName'), params.get('client_password'), productType, params.get('totalAmount'), params.get('CardNumber'), params.get('cardHolderName'), params.get('expiredMonth'), params.get('expiredYear'), params.get('cardCVV'), params.get('client_username'), params.get('users'),'1',str(self.dt))
                cur.execute(query)
                conn.commit()
    ...
    def getUserLimit(self, conn, databaseName):
        try:
            resp = None
            cur = conn.cursor()
            que = cur.execute("SELECT user_limit, created_user FROM %s WHERE databasename = '%s'"%(self.res_usrinfo, databaseName))
            resp = cur.fetchall()

        except Exception as er:
            logging.warning("[WARNING] GETUSERS LIMIT EXCEPTION :: %s"%er)

        return resp

    def usrInfoNumberCount(self, conn, consName, uptdValue):
        try:
            resp = None
            cur = conn.cursor()
            updateque = cur.execute("UPDATE %s SET created_user = %s WHERE databasename = '%s';"%(self.res_usrinfo, uptdValue, consName))
            conn.commit()

        except Exception as er:
            logging.warning("[WARNING] NEW USER CREATION UPDATED COUNT VALUE EXCEPTION : %s"%er)
