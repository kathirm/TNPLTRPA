...
@app.route('/template/pt.json', methods=['POST', 'GET'])
def templateRead():
    try:
        getDir = os.getcwd()
        #with open(getDir+'/template/pt.json') as f:
	with open('/etc/pt.json') as f:
            data = json.load(f) 
    except Exception as er:
        print "Read Language Exception error:: %s"%er
    return jsonify(data)
    ...
    try:
        if request.method=='POST': 
            ...
            kwargs = {
                    ...
                    'language' : request.form["seleclang"]
                    }
            data = json.dumps(kwargs) 
...
def getCurrentstatus():
    try:
        if request.method == "GET":
            resp = None
            resp = requests.get(server+'/getCurrentstatus')
            resp = resp.text
    except Exception as er:
        print("GET CURRENT STATUS EXCEPTION :: %s"%er)
    return resp
...
def random_pwd_generation():
    try:
        ...
        print("random password generation error :: %s"%er)
    return password
...
def payment():
    try:
        if request.method == 'POST':
            random_pwd = random_pwd_generation()
...
if __name__ == "__main__":
    ...
    #app.run(host='10.6.7.88', port=5000)
